...
public class GreatCodeOff2021 {
    public int solution(int N, int K, int[] A, int[] B, int[] C) {
        ...
        for(int i = 0 ; i < M ; i ++) {
            ...
            for(int j = start ; j <= end; j++) {
                if(state[j] == (short)-1) continue;
                if(state[j] == (short)(C[i] - 1)) state[j] = (short)C[i];
                else {
                    state[j] = (short)-1;
                }
            }
    ...
    private static class RangeManager {

    }

    // Best answer
    private final static int DIFFERENT_COLORS = -1;
    public int solution1(int N, int K, int[] A, int[] B, int[] C) {
